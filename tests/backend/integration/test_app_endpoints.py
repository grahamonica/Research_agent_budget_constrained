...
@pytest.fixture
def no_background_n8n(monkeypatch: pytest.MonkeyPatch) -> None:
    async def fake_trigger(_session_id: str, _body: Any) -> None:
        return None

    monkeypatch.setattr(app_module, "_trigger_n8n", fake_trigger)


@pytest.mark.asyncio
async def test_create_session_returns_snapshot_and_stores_session(
    no_background_n8n: None,
) -> None:
...
@pytest.mark.asyncio
async def test_missing_session_endpoints_return_404(
    no_background_n8n: None,
) -> None:
    transport = httpx.ASGITransport(app=app_module.app)
    async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
